...
class MaskedAutoencoderCvT(nn.Module):
    ...
    def forward_inference_details(self, imgs, targets, grad_mask=None, mask_ratio=0.75, do_erosion=True):
        """
        Return reconstructions, maps, and scalar scores for visualization/debugging.

        imgs:     model input, e.g. 9-channel stacked frames if input_3d=True
        targets:  4-channel target = RGB current frame + zero mask channel
        grad_mask: motion gradient image
        """
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio, grad_mask)
        ...
        if self.abnormal_score_func_TS == "L1":
            ts_patch_map = torch.abs(pred_teacher - pred_stud).mean(dim=2)        # [B, L]
            teacher_patch_map = torch.abs(target_patches - pred_teacher).mean(dim=2)
            ts_img_map = self.unpatchify(torch.abs(pred_teacher - pred_stud))
            teacher_img_map = self.unpatchify(torch.abs(target_patches - pred_teacher))
        else:  # L2
            ts_patch_map = ((pred_teacher - pred_stud) ** 2).mean(dim=2)          # [B, L]
            teacher_patch_map = ((target_patches - pred_teacher) ** 2).mean(dim=2)
            ts_img_map = self.unpatchify((pred_teacher - pred_stud) ** 2)
            teacher_img_map = self.unpatchify((target_patches - pred_teacher) ** 2)

        ts_score = ts_patch_map.mean(dim=1)
        teacher_score = teacher_patch_map.mean(dim=1)

        student_recon = self.unpatchify(pred_stud)       # [B, 4, H, W]
        teacher_recon = self.unpatchify(pred_teacher)    # [B, 4, H, W]
        target_recon = self.unpatchify(target_patches)   # [B, 4, H, W]

        ts_patch_map_processed = self.process_result(
            grad_mask, pred_stud, pred_teacher, do_erosion=do_erosion
        )

        return {
            "pred_stud_patches": pred_stud,
            "pred_teacher_patches": pred_teacher,
            "target_patches": target_patches,
            "mask": mask,
            "student_recon": student_recon,
            "teacher_recon": teacher_recon,
            "target_recon": target_recon,
            "ts_patch_map": ts_patch_map,
            "teacher_patch_map": teacher_patch_map,
            "ts_patch_map_processed": ts_patch_map_processed,
            "ts_img_map": ts_img_map,
            "teacher_img_map": teacher_img_map,
            "ts_score": ts_score,
