...
def main():
    ...
    # ── filter videos using ucf_test_gt ─────────────────────────────
    gt_dir = os.path.join(args.data_dir, "gt")

    # get video names that have GT
    allowed_videos = set()
    for f in os.listdir(gt_dir):
        name = os.path.splitext(f)[0]  # remove .npy or .txt
        allowed_videos.add(name)

    print(f"Videos with GT: {len(allowed_videos)}")
    ...
    for name, path in score_files.items():
        ...
        # ── filter only videos with GT ─────────────────────────────
        mask = np.isin(videos_eval, list(allowed_videos))

        labels_eval = labels_eval[mask]
        scores      = scores[mask]
        videos_eval = videos_eval[mask]

        print(f"  After GT filter: {len(np.unique(videos_eval))} videos, {len(labels_eval)} frames")
